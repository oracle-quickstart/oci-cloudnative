...
```bash
kubectl -n mushop get pods -o wide
```
```bash
kubectl -n mushop get nodes -o wide
```
```
...
```
Wait for few minutes for the agent config to get uploaded onto the worker nodes. Impatient users can restart the unified-monitoring-agent_config_downloader.timer service to download the config immediately.