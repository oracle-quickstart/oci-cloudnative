...
public class OrderServiceTest {
    ...
    public void normalOrdersSucceed() throws IOException, InterruptedException {
        ...
        PaymentResponse paymentResponse = new PaymentResponse(true, "Payment authorized");
        ...
        when(asyncGetService.getObject(orderPayload.address, new ParameterizedTypeReference<Address>() {
        })).thenReturn(CompletableFuture.completedFuture(address));

        when(asyncGetService.postResource(
                config.getPaymentUri(),
                paymentRequest,
                new ParameterizedTypeReference<PaymentResponse>() {
                })).thenReturn(CompletableFuture.completedFuture(paymentResponse));

        when(asyncGetService.getObject(orderPayload.card, new ParameterizedTypeReference<Card>() {
        })).thenReturn(CompletableFuture.completedFuture(card));

        when(asyncGetService.getObject(orderPayload.customer, new ParameterizedTypeReference<Customer>() {
        })).thenReturn(CompletableFuture.completedFuture(customer));

        when(asyncGetService.getDataList(orderPayload.items, new
                ParameterizedTypeReference<List<Item>>() {
                })).thenReturn(CompletableFuture.completedFuture(items));

        when(customerOrderRepository.save(any(CustomerOrder.class))).then(returnsFirstArg());

        assertThat(ordersService.createNewOrder(orderPayload)).isInstanceOf(CustomerOrder.class);
    ...
    public void highValueOrdersDeclied() throws IOException, InterruptedException {
        ...
        List<Item> items = Arrays.asList(new Item("001", "001", 1, 200f));
        PaymentRequest paymentRequest = new PaymentRequest(address, card, customer, 204.99f);
        PaymentResponse paymentResponse = new PaymentResponse(false, "Payment unauthorized");
        ...
        assertThrows(OrdersController.PaymentDeclinedException.class, () -> ordersService.createNewOrder(orderPayload));
    }
    ...
    public void ioException_rethrown_as_IllegalStateException() throws IOException, InterruptedException {
        ...
        assertThrows(IllegalStateException.class, () -> ordersService.createNewOrder(orderPayload));
    }
