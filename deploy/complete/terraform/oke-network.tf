...
resource "oci_core_route_table" "oke_route_table" {
  ...
  route_rules {
    ...
    network_entity_id = (var.cluster_workers_visibility == "Private") ? oci_core_nat_gateway.oke_nat_gateway[0].id : oci_core_internet_gateway.oke_internet_gateway[0].id
  }
